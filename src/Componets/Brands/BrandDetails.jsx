...
const BrandDetails = () => {
  ...
  return (
    <main className="bg-[#fffaf6]">
      ...
      {/* Available Sizes & Description Hub */}
      <section id="available-sizes" className="border-t border-[#ebdcd0] bg-[#fffdfa] px-[5%] py-16 sm:py-20">
        <div className="mx-auto max-w-[1380px]">
          {/* Section Header with Tabs */}
          <div className="flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between border-b border-[#ebdcd0] pb-6">
            <div>
            ...
            {/* Custom Tab Switcher */}
            <div className="inline-flex rounded-xl bg-[#f5ece4] p-1.5 shadow-inner self-start sm:self-auto">
              <button
                type="button"
                onClick={() => setActiveTab("sizes")}
                className={`px-6 py-2.5 text-xs sm:text-sm font-extrabold uppercase tracking-wider rounded-lg transition-all duration-300 cursor-pointer ${
                  activeTab === "sizes"
                    ? "bg-[#e96512] text-white shadow-md shadow-[#e96512]/30 scale-[1.02]"
                    : "text-[#6d645e] hover:text-[#282321] hover:bg-white/50"
                }`}
              >
                Available Sizes
              </button>
          ...
          {activeTab === "sizes" && (
            <div className="mt-8 space-y-8 animate-fadeIn">
              ...
              {/* 5 Visual Specification Cards (Different Modern Design) */}
              <div>
          ...
          {/* Tab 2: Description */}
          {activeTab === "description" && (
            <div className="mt-8 rounded-2xl border border-[#ebdcd0] bg-white p-6 sm:p-10 shadow-sm animate-fadeIn">
              <div className="grid gap-8 lg:grid-cols-[1.2fr_0.8fr]">
                <div>
                  <span className="text-[0.68rem] font-extrabold uppercase tracking-[0.2em] text-[#d94c16]">
                    Formulation Overview
                  </span>
                  <h3 className="mt-1 text-2xl sm:text-3xl font-extrabold text-[#282321]">
                    About {brand.name}
                  </h3>
                  <p className="mt-4 text-base leading-8 text-[#766e68]">
                    {brand.description}
                  </p>

                  <div className="mt-8 border-t border-[#f0e7df] pt-6">
                    <h4 className="text-sm font-extrabold uppercase tracking-wider text-[#282321]">
                      Core Performance Highlights
                    </h4>
                    <div className="mt-4 grid gap-3.5 sm:grid-cols-2">
                      <div className="flex items-center gap-2.5 rounded-lg border border-[#f0e7df] bg-[#fffaf6] p-3 text-sm font-bold text-[#4f4843]">
                        <CheckCircle2 size={18} className="text-[#e96512] shrink-0" />
                        <span>Enhanced Gloss & Flow Leveling</span>
                      </div>
                      <div className="flex items-center gap-2.5 rounded-lg border border-[#f0e7df] bg-[#fffaf6] p-3 text-sm font-bold text-[#4f4843]">
                        <CheckCircle2 size={18} className="text-[#e96512] shrink-0" />
                        <span>Fast & Controlled Evaporation</span>
                      </div>
                      <div className="flex items-center gap-2.5 rounded-lg border border-[#f0e7df] bg-[#fffaf6] p-3 text-sm font-bold text-[#4f4843]">
                        <CheckCircle2 size={18} className="text-[#e96512] shrink-0" />
                        <span>Leaves Zero Greasy Residue</span>
                      </div>
                      <div className="flex items-center gap-2.5 rounded-lg border border-[#f0e7df] bg-[#fffaf6] p-3 text-sm font-bold text-[#4f4843]">
                        <CheckCircle2 size={18} className="text-[#e96512] shrink-0" />
                        <span>Optimized User Safety Standards</span>
                      </div>
                    </div>
                  </div>
                </div>

                <div className="rounded-xl bg-[linear-gradient(145deg,#fff8f2_0%,#fdedde_100%)] border border-[#f3dfcf] p-6 sm:p-7 flex flex-col justify-between">
                  <div>
                    <span className="text-[0.68rem] font-extrabold uppercase tracking-[0.16em] text-[#e96512]">
                      Technical Specifications
                    </span>
                    <h4 className="mt-1 text-xl font-extrabold text-[#282321]">
                      Quick Reference
                    </h4>
                    <ul className="mt-5 space-y-3.5 text-sm text-[#766e68]">
                      <li className="flex justify-between border-b border-[#ebdcd0] pb-2.5">
                        <span className="font-semibold">Brand Code:</span>
                        <span className="font-extrabold text-[#282321]">{brand.code}</span>
                      </li>
                      <li className="flex justify-between border-b border-[#ebdcd0] pb-2.5">
                        <span className="font-semibold">Category:</span>
                        <span className="font-extrabold text-[#282321]">{brand.category}</span>
                      </li>
                      <li className="flex justify-between border-b border-[#ebdcd0] pb-2.5">
                        <span className="font-semibold">Standard Pack Sizes:</span>
                        <span className="font-extrabold text-[#282321]">{brand.available_sizes.length} Options</span>
                      </li>
                      <li className="flex justify-between border-b border-[#ebdcd0] pb-2.5">
                        <span className="font-semibold">Container Type:</span>
                        <span className="font-extrabold text-[#282321]">Durable Sealed Cans</span>
                      </li>
                      <li className="flex justify-between">
                        <span className="font-semibold">Custom OEM Filling:</span>
                  ...
                  <Link
                    to="/contact"
                    className="mt-8 inline-flex w-full items-center justify-center gap-2 rounded-lg bg-[#282321] py-3.5 text-xs font-extrabold uppercase tracking-wider text-white shadow-sm transition hover:bg-[#d60e1e]"
                  >
                    Enquire About Formulation <ArrowRight size={14} />
                  </Link>
                </div>
              </div>
            </div>
